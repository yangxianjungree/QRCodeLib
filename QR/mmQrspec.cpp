...
int mmQrspec::MQRspec_getDataLengthBit(int version, QRecLevel level)
{
	int w;
	int ecc;
...
int mmQrspec::MQRspec_getDataLength(int version, QRecLevel level)
{
	return (MQRspec_getDataLengthBit(version, level) + 4) / 8;
}

int mmQrspec::MQRspec_getECCLength(int version, QRecLevel level)
{
	return mqrspecCapacity[version].ec[level];
}

int mmQrspec::MQRspec_getWidth(int version)
{
...
int mmQrspec::MQRspec_lengthIndicator(QRencodeMode mode, int version)
{
	return lengthTableBits[mode][version - 1];
}

int mmQrspec::MQRspec_maximumWords(QRencodeMode mode, int version)
{
	int bits;
	int words;
...
unsigned int mmQrspec::MQRspec_getFormatInfo(int mask, int version, QRecLevel level)
{
	int type;
...
unsigned char *mmQrspec::MQRspec_newFrame(int version)
{
	unsigned char *frame;
	int width;
...
void mmQrspec::MQRspec_clearCache(void)
{
	int i;

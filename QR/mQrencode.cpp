...
static void RSblock_initBlock(RSblock *block, int dl, unsigned char *data, int el, unsigned char *ecc, RS *rs)
{
	...
	mRscode::encode_rs_char(rs, data, ecc);
}
...
static int RSblock_init(RSblock *blocks, int spec[5], unsigned char *data, unsigned char *ecc)
{
	...
	rs = mRscode::init_rs(8, 0x11d, 0, 1, el, 255 - dl - el);
	if(rs == NULL) return -1;
...
__STATIC QRRawCode *QRraw_new(QRinput *input)
{
	...
	raw->datacode = mQrinput::QRinput_getByteStream(input);
	if(raw->datacode == NULL) {
		free(raw);
		return NULL;
	}

	mQrspec::QRspec_getEccSpec(input->version, input->level, spec);
...
__STATIC MQRRawCode *MQRraw_new(QRinput *input)
{
	...
	raw->version = input->version;
	raw->dataLength = mmQrspec::MQRspec_getDataLength(input->version, input->level);
	raw->eccLength = mmQrspec::MQRspec_getECCLength(input->version, input->level);
	raw->oddbits = raw->dataLength * 8 - mmQrspec::MQRspec_getDataLengthBit(input->version, input->level);
	raw->datacode = mQrinput::QRinput_getByteStream(input);
	if(raw->datacode == NULL) {
		free(raw);
		return NULL;
	...
	rs = mRscode::init_rs(8, 0x11d, 0, 1, raw->eccLength, 255 - raw->dataLength - raw->eccLength);
	if(rs == NULL) {
		MQRraw_free(raw);
		return NULL;
	}
...
void mQrencode::QRcode_free(QRcode *qrcode)
{
	if(qrcode != NULL) {
		free(qrcode->data);
		free(qrcode);
...
__STATIC QRcode *QRcode_encodeMask(QRinput *input, int mask)
{
	...
	if(raw == NULL) return NULL;
/*
	mBitStream::ShowByteStream(raw->datacode, raw->dataLength);
	mBitStream::ShowByteStream(raw->ecccode, raw->eccLength);*/

	version = raw->version;
	width = mQrspec::QRspec_getWidth(version);
	frame = mQrspec::QRspec_newFrame(version);
	if(frame == NULL) {
		QRraw_free(raw);
		return NULL;
	}
	...
	//mQrBitMapEncoder Encoder1;
	//		Encoder1.calStructureappendParity(width,width,frame);
	//		Encoder1.Save("d:/testPattern.bmp");

	/* inteleaved data and ecc codes */

	
	//mBitStream::ShowMatrix(frame, width);
	...
	if(mask == -2) { // just for debug purpose
		...
		memcpy(masked, frame, width * width);
			...
			Encoder1.calStructureappendParity(width,width,frame);
			Encoder1.Save("d:/testPattern.bmp");*/
		masked = mMask::Mask_mask(width, frame, input->level);
	} else {
		masked = mMask::Mask_makeMask(width, frame, mask, input->level);
	}
	if(masked == NULL) {
		goto EXIT;
	}
	qrcode = QRcode_new(version, width, masked);
	mBitStream::ShowMatrix(masked, width);
...
__STATIC QRcode *QRcode_encodeMaskMQR(QRinput *input, int mask)
{
	...
	frame = mmQrspec::MQRspec_newFrame(version);
	if(frame == NULL) {
		MQRraw_free(raw);
		return NULL;
	}
	...
	/* masking */
	if(mask < 0) {
		masked = mmMask::MMask_mask(version, frame, input->level);
	} else {
		masked = mmMask::MMask_makeMask(version, frame, mask, input->level);
	}
	if(masked == NULL) {
		goto EXIT;
	}
...
QRcode *mQrencode::QRcode_encodeInput(QRinput *input)
{
	if(input->mqr) {
...
static QRcode *QRcode_encodeStringReal(const char *string, int version, QRecLevel level, int mqr, QRencodeMode hint, int casesensitive)
{
	...
	if(mqr) {
		input = mQrencode::QRinput_newMQR(version, level);
	} else {
		input = mQrencode::QRinput_new2(version, level);
	}
	if(input == NULL) return NULL;

	ret = mSplit::Split_splitStringToQRinput(string, input, hint, casesensitive);
	if(ret < 0) {
		mQrencode::QRinput_free(input);
		return NULL;
	}
	code = mQrencode::QRcode_encodeInput(input);
	mQrencode::QRinput_free(input);

	return code;
}

QRcode *mQrencode::QRcode_encodeString(const char *string, int version, QRecLevel level, QRencodeMode hint, int casesensitive)
{
	return QRcode_encodeStringReal(string, version, level, 0, hint, casesensitive);
}

QRcode *mQrencode::QRcode_encodeStringMQR(const char *string, int version, QRecLevel level, QRencodeMode hint, int casesensitive)
{
...
static QRcode *QRcode_encodeDataReal(const unsigned char *data, int length, int version, QRecLevel level, int mqr)
{
	...
	ret = mQrencode::QRinput_append(input, QR_MODE_8, length, data);
	if(ret < 0) {
		mQrencode::QRinput_free(input);
		return NULL;
	}
...
QRcode *mQrencode::QRcode_encodeData(int size, const unsigned char *data, int version, QRecLevel level)
{
	return QRcode_encodeDataReal(data, size, version, level, 0);
}

QRcode *mQrencode::QRcode_encodeString8bit(const char *string, int version, QRecLevel level)
{
	if(string == NULL) {
		errno = EINVAL;
		return NULL;
...
QRcode *mQrencode::QRcode_encodeDataMQR(int size, const unsigned char *data, int version, QRecLevel level)
{
	return QRcode_encodeDataReal(data, size, version, level, 1);
}

QRcode *mQrencode::QRcode_encodeString8bitMQR(const char *string, int version, QRecLevel level)
{
	if(string == NULL) {
		errno = EINVAL;
		return NULL;
...
{
	if(entry != NULL) {
		mQrencode::QRcode_free(entry->code);
		free(entry);
	}
}

void mQrencode::QRcode_List_free(QRcode_List *qrlist)
{
...
int mQrencode::QRcode_List_size(QRcode_List *qrlist)
{
...
QRcode_List *mQrencode::QRcode_encodeInputStructured(QRinput_Struct *s)
{
...
static QRcode_List *QRcode_encodeInputToStructured(QRinput *input)
{
	...
	s = mQrencode::QRinput_splitQRinputToStruct(input);
	if(s == NULL) return NULL;

	codes = mQrencode::QRcode_encodeInputStructured(s);
	...
	int eightbit, QRencodeMode hint, int casesensitive)
{
	...
	if(eightbit) {
		ret = mQrencode::QRinput_append(input, QR_MODE_8, size, data);
	} else {
		ret = mSplit::Split_splitStringToQRinput((char *)data, input, hint, casesensitive);
	}
	if(ret < 0) {
		mQrencode::QRinput_free(input);
		return NULL;
...
QRcode_List *mQrencode::QRcode_encodeDataStructured(int size, const unsigned char *data, int version, QRecLevel level) {
	return QRcode_encodeDataStructuredReal(size, data, version, level, 1, QR_MODE_NUL, 0);
}

QRcode_List *mQrencode::QRcode_encodeString8bitStructured(const char *string, int version, QRecLevel level) {
	if(string == NULL) {
		errno = EINVAL;
		return NULL;
	}
...
QRcode_List *mQrencode::QRcode_encodeStringStructured(const char *string, int version, QRecLevel level, QRencodeMode hint, int casesensitive)
{
	if(string == NULL) {
		errno = EINVAL;
		return NULL;
...
void mQrencode::QRcode_APIVersion(int *major_version, int *minor_version, int *micro_version)
{
...
char *mQrencode::QRcode_APIVersionString(void)
{
	return VERSION;
}

void mQrencode::QRcode_clearCache(void)
{

...
static int Split_eatAn(const char *string, QRinput *input, QRencodeMode hint)
{
	...
	la = mQrspec::QRspec_lengthIndicator(QR_MODE_AN, input->version);
	ln = mQrspec::QRspec_lengthIndicator(QR_MODE_NUM, input->version);
	...
	while(isalnum(*p)) {
		if(isdigit(*p)) {
			...
			dif = mQrinput::QRinput_estimateBitsModeAn(p - string) /* + 4 + la */
				+ mQrinput::QRinput_estimateBitsModeNum(q - p) + 4 + ln
				- mQrinput::QRinput_estimateBitsModeAn(q - string) /* - 4 - la */;
			if(dif < 0) {
				break;
			} else {
				p = q;
	...
	if(*p && !isalnum(*p)) {
		dif = mQrinput::QRinput_estimateBitsModeAn(run) + 4 + la
			+ mQrinput::QRinput_estimateBitsMode8(1) /* + 4 + l8 */
			- mQrinput::QRinput_estimateBitsMode8(run + 1) /* - 4 - l8 */;
		if(dif > 0) {
			return Split_eat8(string, input, hint);
	...
	ret = mQrencode::QRinput_append(input, QR_MODE_AN, run, (unsigned char *)string);
	if(ret < 0) return -1;
...
static int Split_eatKanji(const char *string, QRinput *input, QRencodeMode hint)
{
	...
	run = p - string;
	ret = mQrencode::QRinput_append(input, QR_MODE_KANJI, run, (unsigned char *)string);
	if(ret < 0) return -1;

	return run;
}

static int Split_eat8(const char *string, QRinput *input, QRencodeMode hint)		//"abcdefg!!!!!!!@#$%hijk123131lmn"
{
	const char *p, *q;
	QRencodeMode mode;
	int ret;
		...
		QRencodeMode hint)
{
	...
	mode = Split_identifyMode(string, hint);	// "adfbd2334"->QR_MODE_AN	"234sdfsf"->QR_MODE_NUM
	if(mode == QR_MODE_NUM) {
...
int mSplit::Split_splitStringToQRinput(const char *string, QRinput *input,	//here	string, input, mode8, 1

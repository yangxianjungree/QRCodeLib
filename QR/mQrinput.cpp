 ...
 * Utilities
 *****************************************************************************/
int mQrinput::QRinput_isSplittableMode(QRencodeMode mode)
{
...
static QRinput_List *QRinput_List_newEntry(QRencodeMode mode, int size, const unsigned char *data)
{
	...
	if(mQrencode::QRinput_check(mode, size, data)) {
		errno = EINVAL;
		return NULL;
	}
...
static void QRinput_List_freeEntry(QRinput_List *entry)
{
	if(entry != NULL) {
		free(entry->data);
		mBitStream::BitStream_free(entry->bstream);
		free(entry);
	}
}
...
QRinput *mQrencode::QRinput_new(void)
{
	return QRinput_new2(0, QR_ECLEVEL_L);
}

QRinput *mQrencode::QRinput_new2(int version, QRecLevel level)
{
	QRinput *input;
...
QRinput *mQrencode::QRinput_newMQR(int version, QRecLevel level)
{
	QRinput *input;

	if(version <= 0 || version > MQRSPEC_VERSION_MAX) goto INVALID;
	if((mmQrspec::MQRspec_getECCLength(version, level) == 0)) goto INVALID;
...
int mQrencode::QRinput_getVersion(QRinput *input)
{
	return input->version;
}

int mQrencode::QRinput_setVersion(QRinput *input, int version)
{
...
QRecLevel mQrencode::QRinput_getErrorCorrectionLevel(QRinput *input)
{
	return input->level;
}

int mQrencode::QRinput_setErrorCorrectionLevel(QRinput *input, QRecLevel level)
{
...
int mQrencode::QRinput_setVersionAndErrorCorrectionLevel(QRinput *input, int version, QRecLevel level)
{
	if(input->mqr) {
		if(version <= 0 || version > MQRSPEC_VERSION_MAX) goto INVALID;
		if((mmQrspec::MQRspec_getECCLength(version, level) == 0)) goto INVALID;
...
int mQrencode::QRinput_append(QRinput *input, QRencodeMode mode, int size, const unsigned char *data)
{
	QRinput_List *entry;
...
int mQrencode::QRinput_appendECIheader(QRinput *input, unsigned int ecinum)
{
	...
	return mQrencode::QRinput_append(input, QR_MODE_ECI, 4, data);
}

void mQrencode::QRinput_free(QRinput *input)
{
...
QRinput *mQrinput::QRinput_dup(QRinput *input)
{
	QRinput *n;
	QRinput_List *list, *e;

	if(input->mqr) {
		n = mQrencode::QRinput_newMQR(input->version, input->level);
	} else {
		n = mQrencode::QRinput_new2(input->version, input->level);
	}
	if(n == NULL) return NULL;
 ...
 */
int mQrinput::QRinput_estimateBitsModeNum(int size)
{
	int w;
	int bits;
...
static int QRinput_encodeModeNum(QRinput_List *entry, int version, int mqr)
{
	...
	entry->bstream = mBitStream::BitStream_new();
	if(entry->bstream == NULL) return -1;

	if(mqr) {
		if(version > 1) {
			ret = mBitStream::BitStream_appendNum(entry->bstream, version - 1, MQRSPEC_MODEID_NUM);
			if(ret < 0) goto ABORT;
		}
		ret = mBitStream::BitStream_appendNum(entry->bstream, mmQrspec::MQRspec_lengthIndicator(QR_MODE_NUM, version), entry->size);
		if(ret < 0) goto ABORT;
	} else {
		ret = mBitStream::BitStream_appendNum(entry->bstream, 4, QRSPEC_MODEID_NUM);
		if(ret < 0) goto ABORT;
	
		ret = mBitStream::BitStream_appendNum(entry->bstream, mQrspec::QRspec_lengthIndicator(QR_MODE_NUM, version), entry->size);
		if(ret < 0) goto ABORT;
	}
	...
	for(i=0; i<words; i++) {
		...
		ret = mBitStream::BitStream_appendNum(entry->bstream, 10, val);
		if(ret < 0) goto ABORT;
	}

	if(entry->size - words * 3 == 1) {
		val = entry->data[words*3] - '0';
		ret = mBitStream::BitStream_appendNum(entry->bstream, 4, val);
		if(ret < 0) goto ABORT;
	} else if(entry->size - words * 3 == 2) {
		val  = (entry->data[words*3  ] - '0') * 10;
		val += (entry->data[words*3+1] - '0');
		mBitStream::BitStream_appendNum(entry->bstream, 7, val);
		if(ret < 0) goto ABORT;
	}

	//mBitStream::ShowBitStream(entry->bstream);
 ...
 * @return number of bits
 */
int mQrinput::QRinput_estimateBitsModeAn(int size)
{
	int w;
	int bits;
...
static int QRinput_encodeModeAn(QRinput_List *entry, int version, int mqr)
{
	...
	if(mqr) {
		...
		ret = mBitStream::BitStream_appendNum(entry->bstream, version - 1, MQRSPEC_MODEID_AN);
		if(ret < 0) goto ABORT;
		ret = mBitStream::BitStream_appendNum(entry->bstream, mmQrspec::MQRspec_lengthIndicator(QR_MODE_AN, version), entry->size);
		if(ret < 0) goto ABORT;
	} else {
		ret = mBitStream::BitStream_appendNum(entry->bstream, 4, QRSPEC_MODEID_AN);
		if(ret < 0) goto ABORT;
		ret = mBitStream::BitStream_appendNum(entry->bstream, mQrspec::QRspec_lengthIndicator(QR_MODE_AN, version), entry->size);
		if(ret < 0) goto ABORT;
	}
	...
	for(i=0; i<words; i++) {
		...
		ret = mBitStream::BitStream_appendNum(entry->bstream, 11, val);
		if(ret < 0) goto ABORT;
	}
	...
	if(entry->size & 1) {
		...
		ret = mBitStream::BitStream_appendNum(entry->bstream, 6, val);
		if(ret < 0) goto ABORT;
	}
 ...
 * @return number of bits
 */
int mQrinput::QRinput_estimateBitsMode8(int size)
{
	return size * 8;
}
...
static int QRinput_encodeMode8(QRinput_List *entry, int version, int mqr)
{
	...
	if(mqr) {
		...
		ret = mBitStream::BitStream_appendNum(entry->bstream, version - 1, MQRSPEC_MODEID_8);
		if(ret < 0) goto ABORT;
		ret = mBitStream::BitStream_appendNum(entry->bstream, mmQrspec::MQRspec_lengthIndicator(QR_MODE_8, version), entry->size);
		if(ret < 0) goto ABORT;
	} else {
		ret = mBitStream::BitStream_appendNum(entry->bstream, 4, QRSPEC_MODEID_8);
		if(ret < 0) goto ABORT;		
		//mBitStream::ShowBitStream(entry->bstream);

		ret = mBitStream::BitStream_appendNum(entry->bstream, mQrspec::QRspec_lengthIndicator(QR_MODE_8, version), entry->size);
		if(ret < 0) goto ABORT;
		//mBitStream::ShowBitStream(entry->bstream);
	}

	ret = mBitStream::BitStream_appendBytes(entry->bstream, entry->size, entry->data);
 ...
 * @return number of bits
 */
int mQrinput::QRinput_estimateBitsModeKanji(int size)
{
	return (size / 2) * 13;
}
...
static int QRinput_encodeModeKanji(QRinput_List *entry, int version, int mqr)
{
	...
	if(mqr) {
		...
		ret = mBitStream::BitStream_appendNum(entry->bstream, version - 1, MQRSPEC_MODEID_KANJI);
		if(ret < 0) goto ABORT;
		ret = mBitStream::BitStream_appendNum(entry->bstream, mmQrspec::MQRspec_lengthIndicator(QR_MODE_KANJI, version), entry->size/2);
		if(ret < 0) goto ABORT;
	} else {
		ret = mBitStream::BitStream_appendNum(entry->bstream, 4, QRSPEC_MODEID_KANJI);
		if(ret < 0) goto ABORT;
		ret = mBitStream::BitStream_appendNum(entry->bstream, mQrspec::QRspec_lengthIndicator(QR_MODE_KANJI, version), entry->size/2);
		if(ret < 0) goto ABORT;
	}
	...
	for(i=0; i<entry->size; i+=2) {
		...
		ret = mBitStream::BitStream_appendNum(entry->bstream, 13, val);
		if(ret < 0) goto ABORT;
	}
...
static int QRinput_encodeModeStructure(QRinput_List *entry, int mqr)
{
	...
	ret = mBitStream::BitStream_appendNum(entry->bstream, 4, QRSPEC_MODEID_STRUCTURE);
	if(ret < 0) goto ABORT;
	ret = mBitStream::BitStream_appendNum(entry->bstream, 4, entry->data[1] - 1);
	if(ret < 0) goto ABORT;
	ret = mBitStream::BitStream_appendNum(entry->bstream, 4, entry->data[0] - 1);
	if(ret < 0) goto ABORT;
	ret = mBitStream::BitStream_appendNum(entry->bstream, 8, entry->data[2]);
	if(ret < 0) goto ABORT;
...
static int QRinput_encodeModeFNC1Second(QRinput_List *entry, int version)
{
	...
	ret = mBitStream::BitStream_appendNum(entry->bstream, 4, QRSPEC_MODEID_FNC1SECOND);
	if(ret < 0) goto ABORT;
	
	ret = mBitStream::BitStream_appendBytes(entry->bstream, 1, entry->data);
	if(ret < 0) goto ABORT;
...
static int QRinput_encodeModeECI(QRinput_List *entry, int version)
{
	...
	ret = mBitStream::BitStream_appendNum(entry->bstream, 4, QRSPEC_MODEID_ECI);
	if(ret < 0) goto ABORT;
	
	ret = mBitStream::BitStream_appendNum(entry->bstream, words * 8, code);
	if(ret < 0) goto ABORT;
...
int mQrencode::QRinput_check(QRencodeMode mode, int size, const unsigned char *data)
{
...
static int QRinput_estimateBitStreamSizeOfEntry(QRinput_List *entry, int version, int mqr)
{
	...
	switch(entry->mode) {
		case QR_MODE_NUM:
			bits = mQrinput::QRinput_estimateBitsModeNum(entry->size);
			break;
		case QR_MODE_AN:
			bits = mQrinput::QRinput_estimateBitsModeAn(entry->size);
			break;
		case QR_MODE_8:
			bits = mQrinput::QRinput_estimateBitsMode8(entry->size);
			break;
		case QR_MODE_KANJI:
			bits = mQrinput::QRinput_estimateBitsModeKanji(entry->size);
			break;
		case QR_MODE_STRUCTURE:
	...
	if(mqr) {
		l = mQrspec::QRspec_lengthIndicator(entry->mode, version);
		m = version - 1;
		bits += l + m;
	} else {
		l = mQrspec::QRspec_lengthIndicator(entry->mode, version);
...
__STATIC int QRinput_lengthOfCode(QRencodeMode mode, int version, int bits)
{
	...
	payload = bits - 4 - mQrspec::QRspec_lengthIndicator(mode, version);
	switch(mode) {
		case QR_MODE_NUM:
			chunks = payload / 10;
	...
	maxsize = mQrspec::QRspec_maximumWords(mode, version);
	if(size < 0) size = 0;
...
static int QRinput_encodeBitStream(QRinput_List *entry, int version, int mqr)
{
	...
	words = mQrspec::QRspec_maximumWords(entry->mode, version);
	if(words != 0 && entry->size > words) {
		...
		ret = mBitStream::BitStream_append(entry->bstream, st1->bstream);
		if(ret < 0) goto ABORT;
		ret = mBitStream::BitStream_append(entry->bstream, st2->bstream);
		if(ret < 0) goto ABORT;
...
static int QRinput_convertData(QRinput *input)
{
	...
	if(ver > mQrencode::QRinput_getVersion(input)) {
		mQrencode::QRinput_setVersion(input, ver);
	}

	for(;;) {
		bits = QRinput_createBitStream(input);
		if(bits < 0) return -1;
		ver = mQrspec::QRspec_getMinimumVersion((bits + 7) / 8, input->level);
		if(ver < 0) {
			errno = ERANGE;
			return -1;
		} else if(ver > mQrencode::QRinput_getVersion(input)) {
			mQrencode::QRinput_setVersion(input, ver);
		} else {
			break;
		}
...
static int QRinput_appendPaddingBitMQR(BitStream *bstream, QRinput *input)
{
	...
	bits = BitStream_size(bstream);
	maxbits = mmQrspec::MQRspec_getDataLengthBit(input->version, input->level);
	maxwords = maxbits / 8;
	...
	padding = mBitStream::BitStream_new();
	if(padding == NULL) return -1;
	ret = mBitStream::BitStream_appendNum(padding, termbits, 0);
	if(ret < 0) goto DONE;
...
__STATIC BitStream *QRinput_mergeBitStream(QRinput *input)
{
	...
	bstream = mBitStream::BitStream_new();
	if(bstream == NULL) return NULL;

	list = input->head;
	while(list != NULL) {
		ret = mBitStream::BitStream_append(bstream, list->bstream);
		if(ret < 0) {
			mBitStream::BitStream_free(bstream);
			return NULL;
		}
		list = list->next;
	}
...
__STATIC BitStream *QRinput_getBitStream(QRinput *input)
{
	...
	//mBitStream::ShowBitStream(bstream);
...
unsigned char *mQrinput::QRinput_getByteStream(QRinput *input)
{
	BitStream *bstream;
	unsigned char *array;
	...
	array = mBitStream::BitStream_toByte(bstream);
	mBitStream::BitStream_free(bstream);
...
{
	if(entry != NULL) {
		mQrencode::QRinput_free(entry->input);
		free(entry);
	}
}

QRinput_Struct *mQrencode::QRinput_Struct_new(void)
{
	QRinput_Struct *s;
...
void mQrencode::QRinput_Struct_setParity(QRinput_Struct *s, unsigned char parity)
{
	s->parity = (int)parity;
}

int mQrencode::QRinput_Struct_appendInput(QRinput_Struct *s, QRinput *input)
{
	QRinput_InputList *e;
...
void mQrencode::QRinput_Struct_free(QRinput_Struct *s)
{
...
static unsigned char QRinput_Struct_calcParity(QRinput_Struct *s)
{
	...
	mQrencode::QRinput_Struct_setParity(s, parity);
...
QRinput_Struct *mQrencode::QRinput_splitQRinputToStruct(QRinput *input)
{
	QRinput *p;
	QRinput_Struct *s;
	...
	input = mQrinput::QRinput_dup(input);
	if(input == NULL) {
		mQrencode::QRinput_Struct_free(s);
		return NULL;
	}

	QRinput_Struct_setParity(s, QRinput_calcParity(input));
	maxbits = mQrspec::QRspec_getDataLength(input->version, input->level) * 8 - STRUCTURE_HEADER_SIZE;

	if(maxbits <= 0) {
		mQrencode::QRinput_Struct_free(s);
		mQrencode::QRinput_free(input);
		return NULL;
	}
	...
	while(list != NULL) {
		...
		if(bits + nextbits <= maxbits) {
			...
			if(bytes > 0) {
				...
				p = mQrencode::QRinput_new2(input->version, input->level);
	...
	ret = mQrencode::QRinput_Struct_insertStructuredAppendHeaders(s);
	if(ret < 0) {
		mQrencode::QRinput_Struct_free(s);
		return NULL;
	}
...
int mQrencode::QRinput_Struct_insertStructuredAppendHeaders(QRinput_Struct *s)
{
	int num, i;
	QRinput_InputList *list;
...
int mQrencode::QRinput_setFNC1First(QRinput *input)
{
	if(input->mqr) {
		errno = EINVAL;
		return -1;
...
int mQrencode::QRinput_setFNC1Second(QRinput *input, unsigned char appid)
{
	if(input->mqr) {
		errno = EINVAL;
		return -1;
